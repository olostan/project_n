...
## 4. Machine Learning Engineering & Scientific Rigor

### 4.1 Loss Formulations & Mathematical Invariants

#### A. Triplet Margin Loss with Cosine Distance
For metric embedding alignment between anchor ($\mathbf{z}_a$), positive ($\mathbf{z}_p$), and negative ($\mathbf{z}_n$) representations:
$$\mathcal{L}_{triplet} = \max\left(0, \cos(\mathbf{z}_a, \mathbf{z}_n) - \cos(\mathbf{z}_a, \mathbf{z}_p) + m\right)$$
where $m = 0.20$ is the operational margin.

#### B. Multi-Class Cosine Contrastive / InfoNCE
For in-batch self-supervised representation learning:
$$\mathcal{L}_{NCE} = -\log \frac{\exp(\mathbf{z}_i \cdot \mathbf{z}_j / \tau)}{\sum_{k=1}^B \exp(\mathbf{z}_i \cdot \mathbf{z}_k / \tau)}$$
with temperature parameter $\tau = 0.07$.

#### C. Metal Hardware-Accelerated Attention
Always use native Apple Silicon scaled dot-product attention via Metal Performance Shaders:
$$\text{Attention}(Q, K, V) = \text{Softmax}\left(\frac{Q K^T}{\sqrt{d_k}} + M\right) V$$
In Apple MLX, execute strictly via `mx.fast.scaled_dot_product_attention(queries, keys, values, scale=scale, mask=mask)`.

#### D. Calibrated Platt Probability Scaling
Nearest-neighbor distances $d$ are mapped to calibrated settling probabilities:
$$P(\text{settles} \mid \text{action}, d) = \sigma(w \cdot d + b) = \frac{1}{1 + \exp(-(w \cdot d + b))}$$
calibrated via Platt scaling or isotonic regression on forward-chaining validation splits.

#### E. Expected Calibration Error (ECE)
Evaluates confidence calibration across $M = 5$ bins:
$$\text{ECE} = \sum_{m=1}^M \frac{|B_m|}{N} \left| \text{acc}(B_m) - \text{conf}(B_m) \right| \le 0.12$$

#### F. Information Retrieval Metrics (MRR & Top-k Precision)
- **Mean Reciprocal Rank (MRR):**
  $$\text{MRR} = \frac{1}{|Q|} \sum_{i=1}^{|Q|} \frac{1}{\text{rank}_i} \ge 0.65$$
- **Top-k Precision ($P@k$):**
  $$P@k = \frac{\sum_{i=1}^k \mathbb{I}(\text{relevant}_i)}{k} \ge 0.70 \quad (k=3)$$

### 4.2 Data Splitting, Normalization & Leakage Prevention
- **Strict Chronological Forward-Chaining Splits:** Standard random $k$-fold cross-validation is **strictly forbidden** on time-series behavioral records due to high temporal autocorrelation leakage. All splits must enforce $T_{train} < T_{val} < T_{test}$.
- **Zero Preprocessing Leakage:** All data transformers (e.g. z-score feature standardizers, min-max normalizers) must fit parameters ($\mu, \sigma$) **strictly on the training partition**, and then transform validation and test sets using the pre-computed training parameters.
- **Numerical Stability Invariant:** Every normalization or division must include epsilon smoothing ($\epsilon = 10^{-8}$) to prevent Metal MPS kernel crashes on zero-magnitude tensors:
  $$\mathbf{z} = \frac{\mathbf{v}}{\sqrt{\sum_{i=1}^D v_i^2 + 10^{-8}}}$$
- **Gradient Norm Clipping:** All MLX optimization steps must enforce gradient clipping ($\|\mathbf{g}\|_2 \le 1.0$) to avoid gradient explosion during multimodal feature fusion.
- **Deterministic Seed Initialization:** Set explicit pseudo-random seeds in all training and fixture scripts (`mx.random.seed(42)`, `np.random.seed(42)`).

---

## 5. Backend Standards (Python 3.11+ / FastAPI / MLX)

### 5.1 Python Standards & Typing
- **Python Version:** 3.11+ strictly required.
- **Strict Typing:** `mypy --strict` is enforced across all modules:
  - Every function, method, and generator must have full argument and return type annotations.
  - `Any` is prohibited unless wrapping untyped C-extensions or external dynamic APIs (with an explanatory comment).
  - Use Python 3.10+ union syntax (`int | str`, `list[str]`).
  - Use `collections.abc.Mapping` and `collections.abc.Sequence` for covariant interface arguments.

### 5.2 FastAPI & REST Architecture
- **Async I/O:** All network, database, and disk operations must be asynchronous (`async def`). CPU/Metal-heavy computations must be offloaded via `asyncio.to_thread` or background queue workers.
...
- **Error Handling:** Use custom domain exceptions mapped to HTTP error handlers in `server/main.py`. Never leak raw unhandled Python stack traces to HTTP clients.

### 5.3 Real-Time Server-Sent Events (SSE) Channel
...
## 6. Frontend Standards (React 18 / Vite / Tailwind)
...
## 7. Mobile Companion Standards (Flutter / Dart / Riverpod)
...
### 7.2 The Offline-First Encrypted Outbox
...
### 7.3 Code Quality & Riverpod Best Practices
...
## 8. Hermetic Testing, Coverage & Presubmit Quality Gates

### 8.1 Hermetic Testing Mandate
To guarantee that tests run reliably and protect family privacy:
1. **Zero External Network Telemetry:** All tests must run 100% offline without connecting to external servers or cloud APIs.
2. **Zero Raw Biometric Data in Git:** Unit and integration tests must **never** depend on raw home video or audio of Child N. Tests must use deterministic synthetic fixtures:
   - **Audio Fixtures:** Synthetic sine tones, harmonic frequency sweeps (chirps), and Gaussian noise generated programmatically via `numpy` and `scipy.signal`.
   - **Kinematic Fixtures:** Synthetic MediaPipe 33-point skeletal landmark matrices with controlled oscillatory motion (e.g. simulated 2.5 Hz wrist flap).
3. **Ephemeral Test Storage:** All database and vault tests must use in-memory SQLite (`:memory:`) or auto-purging temporary directories (`tempfile.TemporaryDirectory()`).
4. **Hardware Keychain Mocking:** Tests that verify vault workflows must inject a mock `KeychainProvider` that emulates `SecItem` without prompting for macOS user authorization.

### 8.2 Code Coverage Requirements
Every test run evaluates branch and line coverage:
- **Domain Contracts & Psychometric Scorers (`models/`):** Minimum **90% branch coverage**.
- **Signal Extraction & Ingestion Pipelines (`extraction/`):** Minimum **80% line coverage**.
- **Server Routers & Service Orchestration (`server/`):** Minimum **80% line coverage**.
- **Dimensional Contracts & Shape Invariants:** **100% assertion coverage** (`tests/test_shapes.py`).

### 8.3 Presubmit & CI/CD Gates Matrix

```mermaid
flowchart LR
    DEV["Developer / AI Agent Work"] --> PRECOMMIT["1. Local Pre-Commit Hooks<br/>• Ruff lint & format<br/>• Mypy strict<br/>• File size (<= 400 lines)<br/>• Shape contract assertions<br/>• Relative link check"]
    PRECOMMIT --> PREPUSH["2. Local Pre-Push Hooks<br/>• Crossref DOI verification<br/>• Full pytest suite"]
    PREPUSH --> GITHUB["3. Remote GitHub Actions CI<br/>• Linux & macOS runners<br/>• Strict build & typecheck<br/>• Invariant scan"]

    classDef stage fill:#f7fafc,stroke:#4a5568,stroke-width:1.5px;
    class DEV,PRECOMMIT,PREPUSH,GITHUB stage;
```

| Quality Gate | Trigger | Tool / Script | Action on Failure |
| :--- | :--- | :--- | :--- |
| **File Size Ceiling** | `pre-commit` | `tests/check_file_sizes.py` | **Blocks Commit** (exceeds 400 lines) |
| **Code Style & Format** | `pre-commit` | `ruff check` & `ruff format` | **Blocks Commit** (syntax/style error) |
| **Static Type Safety** | `pre-commit` | `mypy --strict` | **Blocks Commit** (type violation) |
| **Forbidden Imports** | `pre-commit` | Invariant 10 scan (`grep`) | **Blocks Commit** (cloud SDK or torch) |
| **Tensor Shapes** | `pre-commit` | `tests/test_shapes.py` | **Blocks Commit** (dimensional drift) |
| **Documentation Links** | `pre-commit` | `tests/check_markdown_links.py` | **Blocks Commit** (broken relative link) |
| **Site Generation** | `pre-commit` | `mkdocs build --strict` | **Blocks Commit** (markdown syntax error) |
| **Academic Citations** | `pre-push` | `tests/verify_citations.py` | **Blocks Push** (unresolved DOI) |
| **Hermetic Test Suite** | `pre-push` & CI | `pytest tests/` | **Blocks Push & PR Merge** |

---

## 9. Summary Checklist for Every Code Change

Before committing or submitting a PR, verify:
- [ ] Is every new or modified file strictly **$\le 400$ lines** (target $150\text{--}300$ lines)?
- [ ] Does the change adhere to the **unidirectional dependency hierarchy** (no circular or reverse imports)?
- [ ] Are all functions, methods, and parameters **strictly typed** with zero implicit `Any`?
- [ ] Are numerical operations stabilized with **$\epsilon = 10^{-8}$** and gradients clipped?
- [ ] Do all tests execute **hermetically** using synthetic fixtures with zero network calls?
