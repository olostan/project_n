...
- **Child Assent & Dissent:** The capture system must respect behavioral dissent (e.g., turning away or covering the camera immediately terminates recording). Camera-free private zones (bathrooms, bedrooms) are enforced in software. Untagged footage is automatically purged after a configurable retention window (default: 90 days, caregiver-configurable).
- **Child Identity & Data Separation:** The project documentation transparently acknowledges the personal parental motivation in the foreword and dedications. However, across all formal engineering specifications, runtime data structures, logs, schemas, and invariants, the participant is formally designated as **Child N**. Strict security and privacy protections are enforced: the system is designed for local-only processing and encrypted storage; biometric telemetry (raw audio, video, skeletal landmarks) is sensitive and strictly confined to encrypted local vaults under hardware-backed keys, with zero external cloud exfiltration. At the database and metadata layers, strict pseudonymization is enforced: all runtime tables, inference logs, and benchmark datasets refer to **Child N** and strip direct personal identifiers (such as family names, clinician names, or medical record numbers). Under no circumstances are raw video/audio recordings, facial embeddings, or clinical diagnostic records committed to Git or transmitted over external networks.
...
- Continuous adaptation requires an explicit promotion gate conforming to `docs/evaluation_protocol.md`:
...
**The system is a caregiver-operated, child-centered communication-support tool respecting child assent and agency, NOT a diagnostic device or intent translator. It must never present an inferred state, pain assessment, or intent as fact.**
...
- **Configured Signal-Deviation Screener:** The automated pipeline inspects acoustic and kinematic features strictly to detect *acute signal deviations* (e.g., extreme pitch excursion exceeding calibrated personal limits, sustained high-amplitude vocalization, or acute physical flinching). When a deviation is detected, the system does not diagnose a medical condition; it presents an immediate prompt: *"Configured signal deviation detected. Please perform your family pediatrician-approved physical safety and comfort check."* If no signal deviation is triggered, the system explicitly communicates: *"No configured signal deviation was detected. This does not assess or exclude pain, illness, or distress."*
- **Human-Controlled Clinical Pain Screening & Exploratory Adaptation:** Standardized pain observation instruments—specifically the Non-Communicating Children's Pain Checklist – Postoperative Version (NCCPC-PV; [Breau et al., 2002](https://doi.org/10.1097/00000542-200203000-00004), *Anesthesiology*; cut-off $\ge 11$ for moderate-to-severe pain across 27 items over a 10-minute observation window) versus the home/residential NCCPC-R ([Breau et al., 2002](https://doi.org/10.1016/S0304-3959(02)00179-3), *Pain*; cut-off $\ge 7$ for pain across 30 items over a 2-hour observation window)—require direct human caregiver observation. Because the 2-hour observation requirement of the NCCPC-R makes it unsuitable for immediate post-episode checks, the acute in-the-moment workflow offered on the caregiver's mobile device adapts the 10-minute, 27-item observation of the NCCPC-PV. **This application outside postoperative acute care is an exploratory adaptation, not a formally validated setting; use of this protocol at home must be explicitly reviewed, selected, and approved by the child's pediatrician.** If the caregiver suspects somatic pain, they complete this checklist manually in the app. Automated models must never synthesize artificial checklist scores or claim automated clinical validation.
- **Signal-Quality Precedence Rule:** Fail-closed signal quality gate breaches (Acoustic SNR < 12 dB, clipping > 5%, mean pose confidence < 0.40, or optical flow > 85 px/s) suppress Layer 2 behavioral pattern matching and dyadic suggestions, but do NOT suppress safety triage: the distress screener reports `screener_status: "not_assessable_low_signal_quality"`, guaranteeing uninterpretable clips are never misreported as normal or safe.
...
## 3. Planned Enforcement & Verification Protocol (To be activated upon Phase implementation)
