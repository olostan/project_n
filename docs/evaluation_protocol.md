...
Because co-regulatory support categories and antecedent contexts (e.g., deep pressure co-regulation, hydration check, auditory sensitivity) are non-mutually exclusive and imbalanced, overall accuracy is banned as a primary metric.
- **Per-Class Precision & Recall:** Computed for every category in the co-regulatory taxonomy.
- **Macro-Averaged F1-Score:**
  $$\text{Macro-F1} = \frac{1}{K} \sum_{k=1}^K \frac{2 \cdot P_k \cdot R_k}{P_k + R_k}$$
  where $K$ is the number of active behavioral/antecedent categories.
